...
load(file='Results_Bintauweights_plots_Peter1315.Rdata')
...
plot(D1s$x,D1s$y,ylim=yrange,type='l',col='black',lwd=2,xlab=' ', #xlab='Phycocyanin',
     ylab='Drift or Diffusion')
points(sigmas$x,sigmas$y,type='l',col='red',lty=2,lwd=2)
abline(h=0,lwd=1,lty=2,col='gray')
text(x=6.5,y=0.27,'B',cex=1.5,font=2)
legend('bottomleft',legend=c('diffusion (as s.d.)','drift'),lty=c(2,1),lwd=c(2,2),seg.len=4,
...
#
plot(xvec.ep[2:100],EPF,type='l',lwd=2,col='black',xlim=xrange,xlab=' ', #xlab='Phycocyanin',
     ylab='Effective Potential')
text(x=6,y=-8.3,'C',cex=1.5,font=2)
#
par(mar=c(3.9, 4.5, 0, 1) + 0.1)
xrange=range(c(ETL[,1],ETR[,1]))
...
text(x=-3.5,y=7,paste('mean = ',round(meanETl,0)),cex=1.4,font=2)
text(x=3.2,y=7,paste('mean = ',round(meanETr,0)),cex=1.4,font=2)
text(x=5,y=290,'D',cex=1.5,font=2)
#
...
text(x=5.4,y=0.009,'E',cex=1.5,font=2)
